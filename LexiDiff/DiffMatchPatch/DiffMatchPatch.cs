/*
 *  GP: 2025-09-20 - Replaced HttpUtility with PercentCodec
...
public class diff_match_patch
{
		...
		foreach (string token in tokens)
		{
			...
			switch (token[0])
			{
				case '+':
					...
					param = PercentCodec.DecodeUri(param);
	...
	public List<Patch> patch_fromText(string textline)
	{
		...
		while (textPointer < text.Length)
		{
			...
			while (textPointer < text.Length)
			{
				...
				line = PercentCodec.DecodeUri(line);
				if (sign == '-')
				{
					// Deletion.
	...
	public static string encodeURI(string str)
	{
		...
		return new StringBuilder(PercentCodec.EncodeUri(str))
